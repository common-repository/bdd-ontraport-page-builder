...
define( 'ONTRAPORT_PAGEBUILDER_VERSION', '1.0.0' );
define( 'ONTRAPORT_PAGEBUILDER_FILE', __FILE__ );
define( 'ONTRAPORT_PAGEBUILDER_PATH', plugin_dir_path( ONTRAPORT_PAGEBUILDER_FILE ) );
define( 'ONTRAPORT_PAGEBUILDER_URL', plugin_dir_url( ONTRAPORT_PAGEBUILDER_FILE ) );

register_activation_hook( ONTRAPORT_PAGEBUILDER_FILE, array( 'ONTRAPORT_PAGEBUILDER', 'activate' ) );
register_deactivation_hook( ONTRAPORT_PAGEBUILDER_FILE, array( 'ONTRAPORT_PAGEBUILDER', 'deactivate' ) );

final class ONTRAPORT_PAGEBUILDER {

	/**
	 * Plugin instance.
	 *
	 * @var ONTRAPORT_PAGEBUILDER
	 * @access private
	 */
	 ...
	 * @access private
	 */
	private $bdd_opbp_membership_levels;

	/**
	 * Get plugin instance.
	 *
	 * @return ONTRAPORT_PAGEBUILDER
	 * @static
	 */
	...
	private function __construct() {
		...
		$this->bdd_opbp_membership_levels = get_transient( 'bdd_opbp_membership_levels' );

		if ( ! $this->bdd_opbp_membership_levels ) {
			self::fetch_membership_levels();
		}
	 ...
	 * @access private
	 */
	private function fetch_membership_levels() {
		$pp = new PilotPress();
		$pp->load_settings();
		...
		set_transient( 'bdd_opbp_membership_levels', $this->opvb_membership_levels );
	}
	...
	private function init_templates() {
		...
		$attributes = array(
			'type' => 'checkbox',
			'heading' => __( 'Show this element to the selected membership levels only.', 'bdd-ontraport-pagebuilder' ),
			'param_name' => 'opvb_allowed_membership',
			'value' => $this->opvb_membership_levels,
			'description' => __( 'Show this element to the selected membership levels only.', 'bdd-ontraport-pagebuilder' ),
			'group' => 'Ontraport',
		);
	...
	public function init() {
		load_plugin_textdomain( 'bdd-ontraport-pagebuilder', false, ONTRAPORT_PAGEBUILDER_PATH . 'languages' );
	}
	...
	function disabled_notice() {
		...
		$vcname = __( 'Visual Composer: Page Builder for WordPress', 'bdd-ontraport-pagebuilder' );
		...
		$vclink = sprintf( __( '<a href="%1$s" target="_blank">%2$s</a>', 'bdd-ontraport-pagebuilder' ), $vcurl, $vcname );

		// Setup PilotPress URL
		$ppname = __( 'PilotPress', 'bdd-ontraport-pagebuilder' );
		...
		$pplink = sprintf( __( '<a href="%1$s" target="_blank">%2$s</a>', 'bdd-ontraport-pagebuilder' ), $ppurl, $ppname );

		// Build error message
		/* translators: %1$s: Visual Composer link
		   translators: %2$s: PilotPress link */
		$disabled_notice = sprintf( __( 'The Ontraport Visual Composer Plugin requires both %1$s and %2$s!', 'bdd-ontraport-pagebuilder' ), $vclink, $pplink );
